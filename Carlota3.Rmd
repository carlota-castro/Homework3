...
We carried out the analysis to find out whether there is indeed a significant difference between the salaries of men and women, and whether the difference is due to discrimination or whether it is based on another, possibly valid, determining factor. 
...
To answer the question, we will start by analysing the key statistical parameters.
...
From the data, we can observe that the confidence intervals do not overlap. Hence, we can say that the difference between the salaries of male and female executives is at least 95% statistically significant.

We will also run a hypothesis testing, assuming a null hypothesis that the mean difference in salaries for mean and women is zero. 
...
From the hypothesis testing, we can confirm that.... CARRY ON WITH THIS!!!!
...
The confidence intervals do not overlap. Hence, we can conclude that there is at least a 95% significant statistical difference between experience of female and male executives.
...
```
As we can see from the perfomed using t.test(), the plot of the null distribution and the p value obtained (which is less than 0.05)
